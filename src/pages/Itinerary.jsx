...
export default function Itinerary({ itinerary, serverResponse, tripData, onBack, onLive, onAI }) {
  ...
  return (
    <main className="itinerary-shell">
      ...
      <section className="summary-panel"><div className="summary-label">✦ WHY THIS PLAN</div><p>{itinerary.aiSummary}</p><small>⌁ {itinerary.estimatedTravelTime}</small><button className="summary-ai-button" onClick={onAI}>Ask Bharat AI where to eat affordably →</button></section>
      ...
      <ExpenseDashboard budget={Number(tripData?.budget || serverResponse?.data?.budget || itinerary.budget || itinerary.remainingBudget + itinerary.totalEstimatedCost)} initialBusCost={tripData?.busCost || 0} tripId={tripData?.tripId || 'demo-trip'} />
