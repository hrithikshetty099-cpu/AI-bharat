...
function formatMinsToHours(minStrOrNum) {
  if (!minStrOrNum) return '—'
  let mins = typeof minStrOrNum === 'number' ? minStrOrNum : parseInt(String(minStrOrNum).replace(/[^\d]/g, ''), 10)
  if (isNaN(mins)) return String(minStrOrNum)
...
export default function LiveTrip({ itinerary, tripData, onBack, onSafety, onAI, onComplete }) {
  ...
  const [googleRouteDetails, setGoogleRouteDetails] = useState(null)
  ...
  const nextPlace = nextActivity ? (indianPlaces.find((place) => place.id === nextActivity.placeId) || {
    name: nextActivity.placeName,
    latitude: nextActivity.latitude,
    longitude: nextActivity.longitude,
  ...
  const displayDistance = googleRouteDetails?.distance || route?.distance || (distance === null ? '—' : `${distance.toFixed(1)} km`)
  const displayEta = googleRouteDetails?.duration || (route?.travelTime ? formatMinsToHours(route.travelTime) : (distance === null ? '—' : formatMinsToHours(Math.max(5, Math.round(distance * 7)))))
  ...
  return <main className="live-trip-page">
    ...
    <section className="live-trip-grid"><div className="live-map-panel"><div className="live-map-canvas" style={{ height: '350px', padding: 0 }}><GoogleMap location={location} nextDestination={nextPlace} itineraryPlaces={activities.map((activity) => indianPlaces.find((place) => place.id === activity.placeId) || { name: activity.placeName, latitude: activity.latitude, longitude: activity.longitude, category: activity.category })} route={route} onRouteUpdate={setGoogleRouteDetails} /></div><div className="map-legend"><span><i className="legend-user" /> Current location</span><span><i className="legend-next" /> Next destination</span><span><i className="legend-route" /> Estimated route</span></div></div><aside className="live-trip-context"><p className="home-eyebrow">NEXT DESTINATION</p><h2>{nextPlace?.name || 'No destination yet'}</h2><p className="next-location">📍 {nextPlace ? `${nextPlace.city}, ${nextPlace.state}` : 'Add an itinerary activity'}</p><div className="next-metrics"><div><small>DISTANCE</small><b>{displayDistance}</b></div><div><small>EST. ETA</small><b>{displayEta}</b></div></div><div className="tracking-status"><span className="status-dot" /> <b>{tracking ? 'Location Tracking ON' : demoTracking ? 'Demo Location active' : 'Location Tracking OFF'}</b><small>{timeAgo(location?.timestamp)}</small></div>{location && <p className="accuracy">Accuracy ±{Math.round(location.accuracy)}m · {location.latitude.toFixed(4)}° N, {location.longitude.toFixed(4)}° E</p>}{route && <div style={{ marginTop: '12px', padding: '10px', background: '#315442', borderLeft: '3px solid #e8753d', fontSize: '10px' }}><span style={{ color: '#f6b783', fontWeight: 'bold', display: 'block', marginBottom: '4px', textTransform: 'uppercase', font: '8px "DM Mono", monospace' }}>Route Guide</span><p style={{ margin: 0, color: '#d4e4d9', lineHeight: 1.4 }}>{route.trafficStatus || 'Proceed along the highlighted route.'} {route.travelTime && `Est. travel time is about ${formatMinsToHours(route.travelTime)}.`}</p></div>}{!tracking && !demoTracking ? <div className="location-actions"><button className="primary-action" onClick={allowLocation}>Allow Location</button><button className="demo-location-button" onClick={startDemo}>Start Demo Location</button></div> : <button className="stop-location-button" onClick={stopTracking}>Stop Location Tracking</button>}{locationError && <div className="location-error" role="alert">{locationError}<button onClick={startDemo}>Enter Location Manually / Demo</button></div>}</aside></section>
